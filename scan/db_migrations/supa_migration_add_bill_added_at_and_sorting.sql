-- Add immutable first-detected timestamp for promoted parcels.
...
CREATE INDEX IF NOT EXISTS idx_bills_added_at
    ON public.bills (added_at DESC);

-- Extend list RPC sorting to support date added while preserving existing filters.
...
BEGIN
    WITH filtered AS (
        ...
        WHERE
            ...
            AND (p_zip = '' OR p_zip IS NULL OR b.apn LIKE p_zip || '%' OR b.location_of_property ILIKE '%' || p_zip || '%')
