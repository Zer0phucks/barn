...
4. Identify any owners or residents who appear to be DECEASED. CyberBackgroundChecks often shows a [Deceased] tag, death date, or obituary references next to names. Count how many distinct people associated with the property appear deceased.
...
- "deceased_count": integer indicating how many current owners or residents appear to be deceased (marked with [Deceased] or similar indicators). If none, output 0.
""")
    return "\n".join(parts)
...
async def run_agent(ctx: PropertyResearchInput, verbose: bool = True) -> tuple[bool, str]:
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        # Track deceased count from the latest response
        dc = obj.get("deceased_count")
        if isinstance(dc, int) and dc > deceased_count:
            deceased_count = dc
        if verbose:
            print(f"Iteration {iteration + 1}: satisfied={satisfied}, next_searches={len(next_searches)}, deceased={deceased_count}")
...
"""
    # Persist deceased_count to Supabase
    if ctx.apn and deceased_count >= 0:
        try:
            import db as _db
            _db.update_bill_fields(ctx.apn, {"deceased_count": deceased_count})
            if verbose:
                print(f"Updated deceased_count={deceased_count} for APN {ctx.apn}")
        except Exception as e:
            if verbose:
                print(f"Warning: could not update deceased_count for {ctx.apn}: {e}")
